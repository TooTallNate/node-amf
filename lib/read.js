...
/**
 * Reads an AMF object or packet from the specified Buffer at the specified offset.
 *
 * @param {Buffer} buffer The Buffer instance to read from.
 * @param {Object|Number} [info={offset: 0, ispacket: false}] "Options" object, or the byte offset to begin reading from.
 * @param {Number} [info.offset=0] byte offset to begin reading from.
 * @param {boolean} [info.isPacket=false] flag specifying if buffer contains a packet or an object
 * @return {Object|Array} The decoded AMF object or packet.
 * @api public
 */
...
function read (buffer, info) {
  ...
  info.isPacket = info.isPacket || false;
  // gets reset to 0 on each `read()` call
  info.byteLength = 0;

  if (info.isPacket) {
    return readPacket(buffer, info);
  } else {
    return readData(buffer, info);
  }
}

function readPacket (buffer, info) {
  var packet = {};
  var headers = [];
  var messages = [];

  var offset = info.offset;
  packet.version = buffer.readUInt16BE(offset);
  bytesUsed(info,2);

  var headerCount = buffer.readUInt16BE(info.offset);
  bytesUsed(info,2);

  for (var i = 0, j = headerCount; i < j; i++) {
    headers.push(readHeader(buffer, info));
  }

  var messageCount = buffer.readUInt16BE(info.offset);
  bytesUsed(info,2);

  for (var i = 0, j = messageCount; i < j; i++) {
    messages.push(readMessage(buffer,info));
  }

  // headerCount/messageCount aren't stored in the packet
  // object since they're equal to the length of the arrays.
  // But maybe that's something worth testing for?
...
function readHeader (buffer, info) {
  var headerName = readString(buffer,info);
  //
  var mustUnderstand = buffer.readUInt8(info.offset) != 0 ? true : false;
  bytesUsed(info, 1);
  var headerLength = buffer.readUInt32BE(info.offset);
  bytesUsed(info,4);
  ...
  var header = {
    ...
    length: headerLength,
    // Header payload is just an AMF0 value-type
    value: readData(buffer, info)
  }

  return header;
}

function readMessage (buffer, info) {
  var targetURI = readString(buffer, info);
  var responseURI = readString(buffer, info);
  var messageLength = buffer.readUInt32BE(info.offset);
  bytesUsed(info,4);
  ...
  var message = {
    ...
    // Message payload is just an AMF0 value-type
    value: readData(buffer, info)
  }
